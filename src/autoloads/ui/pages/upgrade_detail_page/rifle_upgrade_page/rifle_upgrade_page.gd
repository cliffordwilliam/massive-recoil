# Shows all rifle upgrade options.
...
func _hydrate_ui() -> void:
	...
	var weapon: WeaponData = GameState.get_weapon_by_id(GameState.RIFLE_ID)
	if weapon:
...
func _on_scroll_list_item_selected(id: StringName) -> void: # Connected via engine GUI.
	if id == DAMAGE_ID:
		if GameState.try_to_buy_a_weapon_damage_upgrade_by_id(GameState.RIFLE_ID):
			_hydrate_ui()
		# TODO: Play a sound on success and fail later.
	elif id == FIRE_RATE_ID:
		if GameState.try_to_buy_a_weapon_fire_rate_upgrade_by_id(GameState.RIFLE_ID):
			_hydrate_ui()
		# TODO: Play a sound on success and fail later.
	elif id == MAGAZINE_SIZE_ID:
		if GameState.try_to_buy_a_weapon_magazine_size_upgrade_by_id(GameState.RIFLE_ID):
			_hydrate_ui()
		# TODO: Play a sound on success and fail later.
	elif id == RELOAD_SPEED_ID:
		if GameState.try_to_buy_a_weapon_reload_speed_upgrade_by_id(GameState.RIFLE_ID):
			_hydrate_ui()
		# TODO: Play a sound on success and fail later.
