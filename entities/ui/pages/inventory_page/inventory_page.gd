...
func _get_owned_weapons_inv_list_item_instances() -> Array:
	return GameState.get_owned_weapons().map(
		func(d: Dictionary) -> InventoryPageListItem:
			return (d.w.inv_page_list_item_scene.instantiate().set_id(d.i).show_equipped_tag(GameState.get_equipped_weapon_id() == d.i).set_mag_current(d.w.magazine_current) )
	)
